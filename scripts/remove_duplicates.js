...
if (!firebaseConfig.apiKey) {
    console.error("Missing required Firebase Auth configuration. Check .env variables.");
    process.exit(1);
}
...
function scoreProduct(data) {
    ...
    if (Array.isArray(data.sizes) && data.sizes.length > 0) score += 1;
...
async function removeDuplicates() {
    ...
    for (const product of allProducts) {
        if (!product.data || !product.data.name) continue;
